...
module.exports = (uint8Array, { xml = false, transportLayerEncodingLabel, defaultEncoding } = {}) => {
  ...
  if (xml) {
    ...
    if (encoding === null) {
      encoding = prescanHTMLEncoding(uint8Array);
    }
...
function prescanHTMLEncoding(uint8Array) {
  ...
  let encoding = prescanMetaCharset(uint8Array.subarray(0, 1024));
  if (encoding === null) {
    // The XML declaration scan is not subject to the meta prescan's 1024-byte limit.
    encoding = getXMLDeclarationEncoding(uint8Array);
  }
  return encoding;
}
